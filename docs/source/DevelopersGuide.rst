...
If you find a bug in ENGRAM, please create a new ticket on the `issue tracker`_,
...
If you have an idea for an improvement to ENGRAM, create a ticket with type
...
Now you need to make sure that the ``engram`` package is on your PYTHONPATH.
You can do this either by installing ENGRAM::
...
An alternate solution is to install Engram with the *develop* option, this avoids
...
documentation system. Any mention of another ENGRAM module, class, attribute,
...
your code with a code checking program.  Then
...
Once you think your changes are ready to be included in the main ENGRAM repository,
...
For future reference, `Porting to Python 3`_ by Lennart Regebro is an excellent resource.
...
You can use the `pycodestyle`_ program to check the code for PEP 8 conformity.

Please do not use ``from xyz import *``.  This is slow, can lead to
...
Add a new version file, such as :file:`/doc/releases/0.1.0.rst` for the release.

First check that the version string (in :file:`engram/version.py`) is correct.
...
To upload the package to `PyPI`_ (currently Garrett Flynn has the necessary permissions to do this)::
...
.. _`issue tracker`: https://github.com/garrettmflynn/engram/issues
...
.. _`GitHub Repository`: https://github.com/garrettmflynn/engram/
