...
__all__ = ('EngramUserMethods')


class EngramUserMethods(object):
    ...
    Those functions are grouped in the following categories:
        ...
        * Engram control (hemisphere / light...)
    ...
    def screenshot(self, saveas, canvas='main', print_size=None, dpi=300.,
        ...
        canvas : {'main', 'colorbar', 'cross-sections'}
            ...
            canvas where the engram is displayed. Use 'colorbar' to render the
        ...
        autocrop : bool | False
            ...
            space between the engram and the border of the picture.
        ...
        >>> # Define a Engram instance :
        >>> vb = Engram()
        >>> # Export the main engram as a (10cm, 20cm) image at 300 dpi:
        >>> vb.screenshot('main.png', canvas='engram', print_size=(10, 20))
        """
    ...
    # =========================================================================
    # =========================================================================
    def engram_control(self, template=None, hemisphere=None, translucent=None,
                      alpha=None, visible=True):
        """Control the type of engram to use.

        Use this method to switch between several engram templates. Then, you
        ...
        >>> vb.engram_control(template='B3', hemisphere='right')
        >>> # Show the GUI :
        >>> vb.show()

        See also
        --------
        engram_list : Get the list of available mesh engram templates.
        """
        ...
        if hemisphere in ['left', 'both', 'right']:
            safely_set_cbox(self._engram_hemi, hemisphere,
                            [self._fcn_engram_template])
            _engram_update = True
        # Transparent / opaque :
        if isinstance(translucent, bool):
            self._engram_translucent.setChecked(translucent)
            self._fcn_engram_translucent()
        # Opacity :
        if isinstance(alpha, (int, float)):
            self._engram_alpha.setValue(alpha * 100.)
            self._fcn_engram_alpha()
        # Visible :
        self.menuDispEngram.setChecked(visible)
        self._engram_grp.setChecked(visible)
        self._fcn_engram_visible()
        # Update :
        if _engram_update:
            self._fcn_engram_template()

    def engram_list(self):
        """Get the list of available mesh engram templates.

        Returns
        -------
        meshes : list
            List of available mesh engram templates.

        Examples
        --------
        >>> # Define a Engram instance :
        >>> vb = Engram()
        >>> print(vb.engram_list())
        """
    ...
    def add_mesh(self, name, vertices, faces, **kwargs):
        ...
        faces : array_like
            Faces of the mesh.
        kargs : dict | {}
            Supplementar arguments pass to the EngramMesh class.
        """
        # Add mesh to user objects :
        mesh = EngramMesh(vertices=vertices, faces=faces, name=name, **kwargs)
    ...
    def cortical_projection(self, radius=10., project_on='surroundings',
        ...
        surroundings or on region of interest.

        Parameters
        ----------
        radius : float | 10.
            Projection radius.
        project_on : {'surroundings', 'roi'}
            Define on which object to project the sources activity. Choose
            either 'surroundings' for projecting the sources activity onto the
            surroundings or 'roi' to project on region of interest (if defined).
    ...
    def cortical_repartition(self, radius=10., project_on='surroundings',
                             contribute=False, mask_color='orange', **kwargs):
    ...
    def sources_fit_to_vertices(self, name=None, fit_to='surroundings'):
        ...
        fit_to : {'surroundings', 'roi'}
            The object name to fit. Use 'surroundings' or 'roi'.
        """
        obj = self.sources[name] if name is not None else self.sources
        v = self.atlas if fit_to == 'surroundings' else self.roi
