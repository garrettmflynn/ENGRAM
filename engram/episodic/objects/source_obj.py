...
from visbrain.objects._projection import _project_sources_data
from visbrain.objects.roi_obj import RoiObj
from visbrain.utils import (tal2mni, color2vb, normalize, vispy_array,
    ...
    def _update_radius(self,timepoint=None):
        ...
        if not hasattr(self,'radius'):
            if np.unique(self._data).size == 1:
                ...
                self.radius = normalize(self._data.copy(), tomin=self._radius_min,
                                tomax=self._radius_max)

        if timepoint:                     
            self._sources._data['a_size'] = self.radius[:,int(timepoint)] # fs/time
            if not hasattr(self,'_timer'):
                self._timer = visuals.Text(str(int(timepoint)), pos=[0,100,0],
                                bold=True, name='Text',
